...
export const GameProvider = ({ children }: { children: ReactNode }) => {
  ...
  const likeCommunityPost = async (postId: string) => {
    ...
    try {
      ...
      await feedbackAPI.toggleLike(postId, token);
  ...
  const uploadArtwork = async (file: File, title: string, spinData?: SpinResult): Promise<boolean> => {
    try {
      const token = localStorage.getItem('token');
      if (!token) throw new Error('No auth token found');
  ...
  const loadUserUploads = async (userId: string): Promise<UserUpload[]> => {
    try {
      ...
      const res = await uploadAPI.getUserUploads(userId, token);
      return res.uploads || [];
    } catch (error) {
  ...
  const getUserStats = async (userId: string): Promise<UserStats> => {
    try {
      ...
      return await uploadAPI.getUserStats(userId, token);
    } catch (error) {
